...
#define PROC_PATH "/proc/proc_monitor"
...
static int read_monitor_data(monitor_data_t *data)
{
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (sscanf(line, "top_count=%d", &data->top_count) == 1)
            continue;
        ...
        if (idx < 16 &&
            ...
            snprintf(data->top[idx].name,
                    sizeof(data->top[idx].name),
                    "%s",
                    name);
...
static int build_json(char *out,
                      size_t out_sz,
                      const monitor_data_t *d)
{
    size_t off = 0;
